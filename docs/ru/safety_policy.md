...
- **Минимальное требуемое количество свечей**: 200 (захардкожено в `FetchMarketDataJob`)
- **Количество запрашиваемых свечей**: настраивается через `RUNTIME_MARKET_DATA_WINDOW_CANDLES` (по умолчанию 300)
